...
func validateConfig(opts configOptions, flags *pflag.FlagSet) error {
	...
	_, err := api.ConfigQuery(opts.cl, path, orgSlug, nil, pipeline.LocalPipelineVars())
	if err != nil {
		return err
	}
...
func processConfig(opts configOptions, flags *pflag.FlagSet) error {
	...
	response, err := api.ConfigQuery(opts.cl, opts.args[0], orgSlug, params, pipeline.LocalPipelineVars())
	if err != nil {
		return err
	}
